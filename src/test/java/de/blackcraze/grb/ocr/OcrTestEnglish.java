...
public class OcrTestEnglish {
    ...
    public void test_screens_fire_e1() throws Exception {
        ...
        List<Mat> extract = Preprocessor.extract(stream);
        Map<String, Long> stocks = OCR.convertToStocks(extract, Locale.ENGLISH);
        Assert.assertTrue(stocks.keySet().size() == 6);
        Assert.assertEquals(Long.valueOf(2146), stocks.get("SEED"));
        Assert.assertEquals(Long.valueOf(4900), stocks.get("WOOD"));
        Assert.assertEquals(Long.valueOf(12435), stocks.get("LIANA_SEED"));
        Assert.assertEquals(Long.valueOf(843), stocks.get("GRAPE_SEED"));
        Assert.assertEquals(Long.valueOf(184), stocks.get("GRAPE"));
        Assert.assertEquals(Long.valueOf(651), stocks.get("LIANA"));
    }
